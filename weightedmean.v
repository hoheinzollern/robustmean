...
Lemma bound_mean : invariant ->
  (`E_[X | S] - `E_[X' | S])^+2 <= `V_[X | S] * 2 * eps / (2 - eps).
Proof.
move=> Hinv.
...
Lemma bound_empirical_variance_S :
  16 * `V_[X | S] <= `V X' ->
  invariant ->
  \sum_(i in S) C i * P i * tau i <= 1/4 * (1 - eps) * `V X'.
Proof.
move => var16 IC.
...
have Hvar_hat0 : 0 <= `V X' by exact: variance_ge0'.
have Hvar_hat_2_eps : 0 <= `V X' * 2 * eps
  by rewrite -mulrA; apply: mulr_ge0.
have Hvar0 : 0 <= `V_[X | S] by exact: cvariance_ge0'.
have ? := pr_S_gt0 pr_cplt_S low_eps.
(*a6*)
apply (@le_trans _ _ ((1 - eps) * (`V_[X | S] + (`E_[X | S] - `E X')^+2))).
...
(*a7-a8*)
apply (@le_trans _ _ ((1 - eps) * `V X' *
                        (16^-1 + 2 * eps *
                                 ((4 * Num.sqrt (2 - eps))^-1 + (Num.sqrt (1 - eps))^-1)^+2))).
  rewrite -(mulrA (1 - eps)).
  rewrite ler_wpM2l //.
  rewrite [leRHS]mulrDr lerD //; first by rewrite lter_pdivlMr// mulrC.
  rewrite 2!mulrA -(sqr_sqrtr Hvar_hat_2_eps).
  rewrite -exprMn (mulrDr (Num.sqrt (`V X' * 2 * eps))).
  ...
  apply: lerD.
    apply: (@le_trans _ _ (Num.sqrt (`V X' * 2 * eps * (4 * 4 * (2 - eps))^-1))); last first.
...
Lemma bound_empirical_variance_cplt_S :
  16 * `V_[X | S] <= `V X' ->
  invariant ->
  2/3 * `V X' <= \sum_(i in cplt_S) C i * P i * tau i.
Proof.
move => var16 HiC.
have ? := pr_S_gt0 pr_cplt_S low_eps.
...
have -> : \sum_(i in cplt_S) C i * P i * tau i =
  `V X' * (\sum_(i in U) C i * P i) - (\sum_(i in S) C i * P i * tau i).
  rewrite /Var {1}/Ex.
...
apply:(@le_trans _ _ (`V X' * (1 - 3 / 2 * eps) -
...
apply (@le_trans _ _ ((1 - 3 / 2 * eps - 1/4 * (1 - eps)) * `V X')); last first.
  by rewrite mulrBl (mulrC (`V X')) lerD // lerN2 bound_empirical_variance_S.
...
Lemma sq_dev_max_neq0 : 0 < `V X' -> sq_dev_max X PC0 != 0.
Proof.
...
Lemma filter1D_arg_decreasing (C : nneg_finfun U) (var : R) :
  ...
  let WP := wgt PC0 in
  forall K : Rleb (`V (X : {RV WP -> R})) (16 * var) <> true,
